...
# This does something similar to a 'case' using types to differenciate the behavior of the function
...
# The type of stopping criterion is unknown
...
"""
$(TYPEDEF)
Stops if `lhs` *or* `rhs` want to stop. Stops as soon as one of the two wants to stop
...
"""
$(TYPEDEF)
Stops if `lhs` *and* `rhs` want to stop. Stops only when the two want to stop
"""
...
$(TYPEDEF)
Stops if `iter` ≧ `limit`.
For instance, `IterLimit(10) stops at the `
"""
#IterLimit(limit)
